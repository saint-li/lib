...
public class AppToast extends Toast {
    ...
    public static void tShort(CharSequence content) {
        try {
            if (toast == null || isShow) {
                toast = Toast.makeText(AppConfig.getApp(), content, Toast.LENGTH_SHORT);
    ...
    public static void tShort(int contentRes) {
        try {
            if (toast == null || isShow) {
                toast = Toast.makeText(AppConfig.getApp(), contentRes, Toast.LENGTH_SHORT);
                toast.setGravity(Gravity.CENTER, 0, 0);
            } else {
    ...
    public static void tLong(CharSequence content) {
        try {
            if (toast == null || isShow) {
                toast = Toast.makeText(AppConfig.getApp(), content, Toast.LENGTH_LONG);
                toast.setGravity(Gravity.CENTER, 0, 0);
            } else {
                toast.setText(content);
    ...
    public static void tLong(int contentRes) {
        try {
            if (toast == null || isShow) {
                toast = Toast.makeText(AppConfig.getApp(), contentRes, Toast.LENGTH_LONG);
                toast.setGravity(Gravity.CENTER, 0, 0);
            } else {
                toast.setText(contentRes);
                toast.setDuration(Toast.LENGTH_LONG);
